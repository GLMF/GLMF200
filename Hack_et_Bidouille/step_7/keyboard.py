...
def notifySend(title, message, time=1500):
    subprocess.Popen(['notify-send', '-t', str(time), title, message])
...
def startRecording(device):
    ...
    notifySend('Keyboard Macros', 'Start recording macro')
...
def stopRecording(device, recording_buffer):
    ...
    notifySend('Keyboard Macros', 'Stop recording macro\nHit a key to save the buffer')
    return False

def saveMacro(key, recording_buffer, macros):
    if key in macros and 'lock' in macros[key]:
        notifySend('Keyboard Macros', 'Macro on <b>{}</b> is protected'.format(key))
    else:
...
def writeConfig(macros, configFile='keyboardrc.ini'):
    ...
    try:   
        ...
        notifySend('Keyboard Macros', 'Write error on config file <b>{}</b>'.format(configFile))
        exit(2)
    ...
    for event in device.read_loop():
        if event.type == ecodes.EV_KEY:
            ...
            if recording and event.code != key.RECORDING and event.value == 1:
                notifySend('Keyboard Macros', 'Appui sur <b>{}</b>'.format(ecodes.KEY[event.code]), 500)
            ...
            # Set attribute to a macro
                ...
                notifySend('Keyboard Macros', 'Macro saved in <b>{}</b>'.format(ecodes.KEY[event.code]))
                setCapsLock(device, 'off')
                recording_buffer = []
                print(macros)
            # Protect a macro
            elif event.code == ecodes.KEY_TAB and event.value == 1:
                notifySend('Keyboard Macros', 'Hit a key to indicate macro to protect')
                setCapsLock(device, 'on')
                protect = True
            elif protect and event.value == 1:
                if ecodes.KEY[event.code] in macros:
                    protectKey(ecodes.KEY[event.code], macros)
                    notifySend('Keyboard Macros', 'Macro <b>{}</b> is protected'.format(ecodes.KEY[event.code]))
                else:
            ...
            # Unprotect a macro
                ...
                notifySend('Keyboard Macros', 'Hit a key to indicate macro to unprotect')
                setCapsLock(device, 'on')
                unprotect = True
            elif unprotect and event.value == 1:
                if ecodes.KEY[event.code] in macros:
                    unprotectKey(ecodes.KEY[event.code], macros)
                    notifySend('Keyboard Macros', 'Macro <b>{}</b> is no more protected'.format(ecodes.KEY[event.code]))
                else:
                    notifySend('Keyboard Macros', 'Macro not found')
                setCapsLock(device, 'off')
                unprotect = False
...
if __name__ == '__main__':
    ...
    resetLeds(device)
    notifySend('Keyboard Macros', 'Running and connected to <b>{}</b>'.format(dev))
